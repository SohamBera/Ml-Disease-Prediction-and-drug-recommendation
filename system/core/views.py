...
def MakePredict(request):
	...
	all_disease_list = {'fungal infection':0,'allergy':1,'gerd':2,'chronic cholestasis':3,'drug reaction':4,
...
'impetigo':40}
	...
	final_result = ""
	for key, value in all_disease_list.items():
		if value == result:
			print(key)
			final_result = key
	...
	a = Medical(s1=s1, s2=s2, s3=s3, s4=s4, s5=s5, disease=final_result, patient_id=id)
	a.save()

	return JsonResponse({'status':final_result})			
...
def MakeMend(request):
  ...
  if sick in disease_list:
      ...
      clf = joblib.load('model/dt_classifier.pkl')
      prediction = clf.predict(test)
      prediction = prediction[0]
      print('Recommended Drug Is',prediction)

      try:
          check_medical = Medical.objects.filter(patient_id=patient_id, disease=disease).exists()
          if(check_medical == False):
              Medical.objects.filter(patient_id=patient_id, pk=disease).update(medicine=prediction)
              return JsonResponse({'status':prediction})
          else:
              print('Drug Exist')
          return JsonResponse({'status':'Drug Exist'}) 
      except Exception as e:
          print(e)    
  else:
      print('AI Can Not Recommend Drug')
      Medical.objects.filter(pk= disease).update(medicine='See Doctor')
      return JsonResponse({'status':'AI Can Not Recommend Drug'})
